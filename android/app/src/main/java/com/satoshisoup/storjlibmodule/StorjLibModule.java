...
import com.satoshisoup.storjlibmodule.callbackwrappers.RegisterCallbackWrapper;
import com.satoshisoup.storjlibmodule.models.KeyModel;
...
public class StorjLibModule extends ReactContextBaseJavaModule {
    public static final String STORJ_URL = "https://mars.tardigrade.io:7777";
    ...
    @ReactMethod
    public void verifyKeys(final String apikey, final String encryptionKey, final Promise promise) {
        new MethodHandler().invokeParallel(new BaseMethodParams(promise), new IMethodHandlerCallback() {
            @Override
            public void callback(IMethodParams param) throws Exception {
                int error = getStorj().verifyKeys(new Keys(apikey, encryptionKey));
    ...
    @ReactMethod
    public void importKeys(final String apikey, final String encryptionKey, final String passcode, Promise promise) {
        new MethodHandler().invokeParallel(new BaseMethodParams(promise), new IMethodHandlerCallback() {
            @Override
            public void callback(IMethodParams param) throws Exception {
                boolean isSuccess = getStorj().importKeys(new Keys(apikey, encryptionKey), passcode);
